...
export const createCamp = async (req, res, next) => {
  try {
    // Basic required-field check (more forgiving than Joi):
    const body = { ...req.body };
    ...
    // Sanitize/coerce optional fields
    if (body.price !== undefined && body.price !== null && body.price !== "") {
      const parsed = Number(body.price);
      body.price = isNaN(parsed) ? undefined : parsed;
    } else {
      delete body.price;
    }
    ...
    if (body.rating !== undefined && body.rating !== null && body.rating !== "") {
      ...
      body.rating = isNaN(parsed) ? undefined : parsed;
    } else {
      delete body.rating;
    }

    if (body.reviews !== undefined && body.reviews !== null && body.reviews !== "") {
      const parsed = Number(body.reviews);
      body.reviews = isNaN(parsed) ? undefined : parsed;
    } else {
      delete body.reviews;
    }
    ...
    // Keep other fields as-is (description, badge, image, etc.)
    const payload = {
      name,
      location,
      ...body,
    ...
    console.error("createCamp error:", err?.message || err);
...
export const listCamps = async (req, res, next) => {
  try {
    ...
    if (badge) {
      // allow filtering by badge/status (case-insensitive exact match)
